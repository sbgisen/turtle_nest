...
void generate_launch_file(QString workspace_path, QString package_name, QString launch_file_name, QString node_name_cpp, QString node_name_python, BuildType build_type){
...
def generate_launch_description() -> LaunchDescription:
    ...
    oss << R"(
        Node(
            ...
            name=')" << node_name_cpp.toStdString() << R"(',
            output='screen',
        ),)";
    ...
    oss << R"(
        Node(
            ...
            name=')" << node_name_python.toStdString() << R"(',
            output='screen',
        ),)";
    }
    oss << R"(
...
void append_launch_to_cmake(QString c_make_path) {
    QString lines_to_append = R"(# Install launch files
install(DIRECTORY
  launch
...
void append_launch_to_setup_py(QString setup_py_path, QString package_name){
    QString lines_to_append("(os.path.join('share', '%1', 'launch'), glob(os.path.join('launch', '*launch.[pxy][yma]*'))),\n        ");
    lines_to_append = lines_to_append.arg(package_name);
    append_to_file_before(setup_py_path, lines_to_append, "('share/ament_index/resource_index/packages'");
